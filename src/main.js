...
function start(){
  ...
  answered=false;finalScore=0;hideTransient();game.start();updateVerse();resetPassageTimer();
...
function pause(){if(game.pause()){$('paused-overlay').classList.remove('hidden');$('pause-button').innerHTML=icons.play;$('pause-button').setAttribute('aria-label','Resume game');$('resume-button').focus({preventScroll:true});updateVerse();}}
function resume(){if(game.resume()){$('paused-overlay').classList.add('hidden');$('pause-button').innerHTML=icons.pause;$('pause-button').setAttribute('aria-label','Pause game');$('game-stage').focus({preventScroll:true});updateVerse();}}
...
function finish(){hideTransient();finalScore=Math.floor(game.state.score);const record=finalScore>save.best;save.best=Math.max(save.best,finalScore);save.runs++;persist();updateBest();$('pause-button').disabled=true;$('touch-controls').classList.add('hidden');$('end-kicker').textContent=record?'A NEW PERSONAL BEST':'EVERY STEP COUNTS';$('end-score').textContent=number(finalScore);$('end-distance').textContent=Math.floor(game.state.distance)+' m';$('end-combo').textContent='×'+game.state.bestCombo;$('recall-reference').textContent=recallVerse.reference+' · KJV';$('recall-prompt').textContent=recallVerse.recall.prompt;$('recall-feedback').textContent='Complete the verse for a 100-point bonus.';$('recall-options').replaceChildren();
...
$('sound-button').addEventListener('click',async()=>{const enabled=await audio.toggle();$('sound-button').innerHTML=enabled?icons.sound:icons.muted;$('sound-button').setAttribute('aria-pressed',String(enabled));$('sound-button').setAttribute('aria-label',enabled?'Mute sound':'Enable sound');if(enabled)audio.play('collect');else if(!('AudioContext' in window)&&!('webkitAudioContext' in window))toast('Sound is unavailable in this browser.');});
